...
interface Template12Data extends SlideTemplateData {
  title: string;
  quarters: string[];
...
const BAR_W   = 80;
const N_BARS  = 4;
const COL_W   = INNER_W / N_BARS; // 163
const GX      = [0, 1, 2, 3].map(i => PAD_L + COL_W * (i + 0.5));
...
function BusinessTable({
  ...
  const tblCols = `${LABEL_W}px ${16 + marginLeft}px repeat(${N_BARS}, ${COL_W}px) ${PAD_R}px`;
  const totals = [0, 1, 2, 3].map(qi =>
    businesses.reduce((sum, b) => sum + (b.values[qi] ?? 0), 0)
  );
  return (
    <div style={{ width: CHART_W + marginLeft, marginTop: 8, flexShrink: 0 }}>
      {businesses.map(biz => (
        <div key={biz.name} style={{
          ...
          <div />
          {biz.values.slice(0, N_BARS).map((v, i) => {
            const man = Math.round(v / 10_000);
            return (
              <div key={i} style={{ textAlign: "center", fontSize: 20, color: man > 0 ? "#18191e" : "#d1d5db", fontFamily: "Gen Interface JP Display:SemiBold, sans-serif" }}>
      ...
      <div style={{
        ...
        {showLabel ? (
          <div style={{ fontSize: 16, color: "#18191e", fontFamily: "Gen Interface JP Display:SemiBold, sans-serif", whiteSpace: "nowrap" }}>合計</div>
        ) : <div />}
        <div />
        {totals.map((total, i) => {
          const man = Math.round(total / 10_000);
          return (
            <div key={i} style={{ textAlign: "center", fontSize: 20, color: "#18191e", fontFamily: "Gen Interface JP Display:SemiBold, sans-serif" }}>
              {man > 0 ? `${man.toLocaleString()}万` : "—"}
            </div>
          );
        })}
...
export default function Template12({ data }: { data: Template12Data }) {
  ...
  const quarters = data.quarters?.length >= 4 ? data.quarters : ["Q1", "Q2", "Q3", "Q4"];

  return (
    <div className="bg-white relative size-full" data-name="template12">
      <SafeArea>
        ...
        <div className="absolute left-0 top-[96px] bottom-0 flex gap-0">
          <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            <div ref={chartWrapperRef} style={{ flex: 1, minHeight: 0 }}>
              <StackedBarChart quarters={quarters} businesses={data.revenues ?? []} svgMarginLeft={54} chartH={chartH} />
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ flex: 1, minHeight: 0 }}>
              <StackedBarChart quarters={quarters} businesses={data.profits ?? []} chartH={chartH} />
            </div>
